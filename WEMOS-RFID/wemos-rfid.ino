...
void setup()
{
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (WiFi.SSID(i) == ssidIFSLAB) {
      ...
      Serial.print(", com a senha ");
      Serial.println(passwordIFSLAB);
      WiFi.begin(ssidIFSLAB, passwordIFSLAB);
      break;
    }
  }

  // Enquanto não conectar, espera
  while ( WiFi.status() != WL_CONNECTED ) {
    delay(500);
    Serial.print(".");
  }

  // Conectao! Liga o led Verde e apaga o Vermelho
  digitalWrite(redLed, LOW);
  digitalWrite(greenLed, HIGH);

  // Mostra o IP da WeMos no WiFi conectado
